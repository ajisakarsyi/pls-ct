...
All retry logic and LaTeX normalisation live here so no other module
...
logger.info("LLM client initialised — base: %s", _settings.openai_api_base)


def query_llm(prompt: str) -> str:
    """
    Send *prompt* to the chat-completions endpoint and return the
    normalised text response.  Retries up to ``settings.chat_retries``
    times on failure.
    """
