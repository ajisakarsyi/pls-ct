...
def generate_reply(
    ...
    """
    ═══════════════════════════════════════════════════════════════
    FUNGSI INTI TUTOR — generate_reply()
    File: app/services/tutor.py
    ═══════════════════════════════════════════════════════════════

    Q: "Bagaimana alur dari pertanyaan mahasiswa hingga jawaban?"
    A: Lima langkah utama:
       1. Ambil label profil kognitif dari cognitive_code (misal: 3TAR)
       2. Ambil riwayat percakapan sesi untuk konteks multi-turn
       3. Retrieve 6 chunk paling relevan dari FAISS (RAG)
       4. Format semua elemen ke dalam prompt template
       5. Kirim ke LLM → terima jawaban

    Q: "Bagaimana profil kognitif mempengaruhi jawaban LLM?"
    A: Profil kognitif diinjeksikan ke prompt melalui dua mekanisme:
       a. Label deskriptif ({label}) → instruksi gaya penjelasan
          misal: "Jelaskan sesuai gaya kognitif Level 3, Teoritis, Analitis, Reflektif"
       b. Konteks dokumen profil ({context}) → chunk dari file 3TAR.txt
          yang berisi Deskripsi, Ciri Khas, Kelemahan, dan Sugesti Fokus LLM

    Q: "Di mana profil kognitif dibedakan untuk teks biasa vs kode?"
    A: is_code_like(message) → True jika ada sintaks kode
       → CHAT_CODE_PROMPT_TEMPLATE (instruksi berbeda untuk kode)
       → CHAT_PROMPT_TEMPLATE untuk pertanyaan biasa

    Q: "Di mana log interaksi disimpan?"
    A: log_interaction() di app/services/session.py
       → disimpan ke history_logs/ sebagai JSON per sesi
    ═══════════════════════════════════════════════════════════════
    ...
    """
    # Langkah 1: label profil kognitif untuk prompt (misal: "Level 3 — Teoritis, Analitis, Reflektif")
    label       = cognitive_label(cognitive_code)
    history     = get_session(session_id)
    history_txt = format_history(history)

    # Langkah 3: RAG — ambil 6 chunk paling relevan dari FAISS
